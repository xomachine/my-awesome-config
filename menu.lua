...
local myawesomemenu = {
   ...
   { "Проверка", general.terminal .. " -e \"bash -c 'Xephyr -resizeable :1 2>/dev/null & sleep 1s; DISPLAY=:1 awesome;'\""},
...
local systemmenu = {
   { "Переключение видеокарты", "sh '"..os.getenv("HOME").."/.Soft/nvidia_switch.sh'" },
   { "Перезагрузка", "reboot" },
